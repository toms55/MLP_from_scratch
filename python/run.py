...
def custom_train_model_with_history(mlp: MLP, X: np.ndarray, y: np.ndarray, epochs: int) -> List[float]:
    loss_history = []
...
def benchmark_custom_mlp(X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray, epochs: int = 100) -> Dict[str, Any]:
    ...
    loss_history = custom_train_model_with_history(mlp, X_train, y_train, epochs=epochs)
...
def benchmark_sklearn_mlp(X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray, epochs: int = 500) -> Dict[str, Any]:
    print("\n" + "="*60)
...
def plot_loss_history(results: List[Dict[str, Any]]):
    custom_result = next((r for r in results if r['name'] == 'Custom MLP'), None)
    if custom_result and custom_result.get('loss_history'):
        loss_history = custom_result['loss_history']
        epochs_list = range(1, len(loss_history) + 1)

        plt.figure(figsize=(10, 6))
        plt.plot(epochs_list, loss_history, marker='.', linestyle='-', color='#FF6B6B', linewidth=2)
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Training Loss (MSE)', fontsize=12)
        plt.title('Custom MLP Training Loss vs. Epochs', fontsize=14, fontweight='bold')
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.xticks(np.linspace(1, len(loss_history), num=min(10, len(loss_history)), dtype=int))
        plt.tight_layout()
        plt.savefig('mlp_custom_loss_history.png', dpi=300, bbox_inches='tight')
        plt.show()
    else:
        print("Loss history not available for Custom MLP.")
